...
class AddNewFragment : Fragment(R.layout.fragment_add_news) {
    ...
    private val activityResultAddManyPhoto =
    ...
    // FIXME -->> Can not perform this action after onSaveInstanceState
    // сломался кроп
    ...
    private val activityResultAddTitlePhoto =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
            if (it.resultCode == Activity.RESULT_OK) {
    ...
    private fun setImage(image: Uri) {
        ...
        binding.imageTitleAddNew.setPadding(0, 0, 0, 0)
    ...
    private fun checkEmpty(): Boolean {
        ...
        if (binding.titleText.text?.trim()?.isNotEmpty() != true) {
            ...
            binding.titleContainer.error = "Введите заголовок"
        }

        if (binding.bodyText.text?.trim()?.isNotEmpty() != true) {
            listChecks.add(false)
            binding.bodyContainer.error = "Введите текст новости"
        }
